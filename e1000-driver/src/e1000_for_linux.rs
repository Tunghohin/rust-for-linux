...
impl E1000Driver {
    fn handle_rx_irq(dev: &net::Device, napi: &Napi, data: &NetData) {
        ...
        let mut e1000_device = data.dev_e1000.lock();
        let rx_buf = e1000_device.as_mut().unwrap().e1000_recv();

        if let Some(rx_buf) = rx_buf {
            pkts_compl = rx_buf.len();
            for (i, pkt) in rx_buf.into_iter().enumerate() {
                let pkt_len = pkt.len();
                let skb = dev.alloc_skb_ip_align(RXBUFFER).unwrap();
                let skb_buf =
                    unsafe { from_raw_parts_mut(skb.head_data().as_ptr() as *mut u8, pkt_len) };
                skb_buf.copy_from_slice(&pkt);
...
impl net::DeviceOperations for E1000Driver {
    ...
    fn open(dev: &Device, data: &NetData) -> Result {
        ...
        let res = request_irq(data.irq, irq_data).unwrap();
        data.irq_handler
            .store(Box::into_raw(Box::try_new(res).unwrap()), Ordering::Relaxed);
    ...
    fn start_xmit(
        ...
        data.stats
            .tx_bytes
            .fetch_add(skb.len().into(), Ordering::Relaxed);
        data.stats.tx_packets.fetch_add(1, Ordering::Release);
        dev.completed_queue(1, skb.len().into());
